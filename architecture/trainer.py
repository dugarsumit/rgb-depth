...
class Trainer(object):
    ...
    def __init__(self, model, optim = torch.optim.Adam, loss_func = torch.nn.MSELoss(), args_adam = {},
                augmentation_list = ['rotate180', 'fliplr', 'flipud', 'invert_colors'], reg = 1, train_upsampler = False, cuda=False, accuracy_thresholds = 1.5):
        self.model = model
        ...
        if self.train_upsampler:
            self.freeze_network()
            self.optim_list = [{'params': model.scale_3_section_1.parameters(), 'lr': 1e-3, 'weight_decay': weight_decay},
                                {'params': model.scale_3_section_2.parameters(), 'lr': 1e-2, 'weight_decay': weight_decay},
                                {'params': model.scale_3_section_3.parameters(), 'lr': 1e-2, 'weight_decay': weight_decay},
                                {'params': model.scale_3_section_4.parameters(), 'lr': 1e-3, 'weight_decay': weight_decay}]
        else:
            self.optim_list = [{'params':model.scale_1_section_1.parameters(),'lr': 1e-4, 'weight_decay': weight_decay},
                           ...
                           {'params':model.scale_1_section_4.parameters(),'lr': 1e-1, 'weight_decay': weight_decay},
                           ...
                           {'params':model.scale_2_section_5.parameters(),'lr': 1e-3, 'weight_decay': weight_decay}]
    ...
    def train(self, result_file, train_loader, val_loader, save_model_path, loss_graph_path, acc_graph_path, result_img_path = None, num_epochs = 10,
        ...
        for epoch in range(num_epochs):
            ...
            #print("changing lrs: ", optim.state_dict())
            ...
            # self.val_acc_history.append(compute_val_accuracy(val_loader, model))
            # print_current_accuracy(epoch = epoch,
            #                        num_epochs = num_epochs,
